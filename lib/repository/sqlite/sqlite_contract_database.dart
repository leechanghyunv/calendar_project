...
class ContractDatabase {
  ...
  Future<void> updateOrder(List<LabourCondition> items) async {
    final target = items.last;
    await database.transaction((txn) async {
      await txn.delete('labour_condition', where: 'id = ?', whereArgs: [target.id]);
      final map = target.toMap()..remove('id');
      await txn.insert('labour_condition', map);
    });
  }
